...
function GetLifeSupportRating(data) {
	...
	for (let i = 0; i < commonBits.length; i++) {
		dataOxy = dataOxy.filter(bytes => bytes[i] === commonBits[i]);
		commonBits = GetBytesPopularity(dataOxy).commonBits;
		if (dataOxy.length == 1) {
			dataOxy = parseInt(dataOxy[0].join(""), 2);
			break;
		}
	}

	for (let i = 0; i < rareBits.length; i++) {
		dataCO2 = dataCO2.filter(bytes => bytes[i] === rareBits[i]);
		rareBits = GetBytesPopularity(dataCO2).rareBits;
		if (dataCO2.length == 1) {
			dataCO2 = parseInt(dataCO2[0].join(""), 2);
			break;
		}
	}
